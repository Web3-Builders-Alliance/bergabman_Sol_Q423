...
use anchor_lang::{
    prelude::*,
    solana_program::{compute_units::sol_remaining_compute_units, log::sol_log_compute_units},
};
...
impl<'info> Deploy<'info> {
    ...
    pub fn decompress_data(&mut self) -> Result<()> {
        ...
        for i in 0..offsets_5_len {
            offsets_5_index += 1;
            this_offset = u24::from_le_bytes(*array_ref![offsets_5, i as usize * 3, 3]).into();
            ...
            if sol_remaining_compute_units() < 30_000 || sol_remaining_compute_units() > 1_370_000 {
                ...
                if sol_remaining_compute_units() < 10_000 {
                    ...
                    msg!(
                        "current offsets5 index {} from {}",
                        offsets_5_index,
                        self.dev_config.ot_5_len
                    );
                    msg!(
                        "current offsets6 index {} from {}",
                        offsets_6_index,
                        self.dev_config.ot_6_len
                    );
                    break;
                }
            }
